...
from zfssa_utils.common import HEADER, response_size, read_yaml_file, \
     createprogress, createlogger, PROJECTLOGFILE
    ...
    if createproject:
        if args.progress:
            ...
            for entry in projectlistfromfile:
                err, msg = create_project(entry, zfsurl, zauth)
                if err:
                    logger.warning(msg)
                else:
                    logger.info(msg)
        ...
        if args.progress:
            ...
            logger = createlogger(PROJECTLOGFILE)
            for entry in projectlistfromfile:
                err, msg = delete_project(entry, zfsurl, zauth)
                if err:
                    logger.warning(msg)
                else:
                    logger.info(msg)
            ...
            print("#" * 79)
            for entry in projectlistfromfile:
                print(delete_project(entry, zfsurl, zauth)[1])
                print("=" * 79)
    elif listprojects:
        if args.progress:
            progbar = createprogress(len(projectlistfromfile))
            logger = createlogger(PROJECTLOGFILE)
            for entry in projectlistfromfile:
                err, msg = list_projects(entry, zfsurl, zauth)
                initial += 1
                if err:
                    logger.warning(msg)
                else:
